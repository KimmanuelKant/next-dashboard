...
import { FplLeagueStandings, FplLeagueStandingResult } from '@/types/fpl/FplLeagueStandings';
...
export async function computeLeaguePlayerStats(
  ...
  const leagueEntries: FplLeagueStandingResult[] = leagueStandings.standings.results;
  ...
  allFplPlayers.forEach((player) => {
    ...
    const validPercentage = isNaN(globalOwnershipPercentage) ? 0 : globalOwnershipPercentage;
    const globalOwnershipCount = Math.round((validPercentage / 100) * totalPlayers);
  ...
  await Promise.all(leagueEntries.map(async (entry) => {
    try {
      const picksData = await getEntryEventPicks(entry.entry, latestFinishedGameweek);
      ...
      console.warn(`Failed to fetch picks for entry ${entry.entry}, GW ${latestFinishedGameweek}: ${err}`);
    }
  }));
